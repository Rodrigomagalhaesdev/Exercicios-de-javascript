...
const haveLeonOrNot = animals.includes("leão");
...
console.log(`A soma final é ${sum}`);
...
let finalSentence = sentence.reduce((acc, word) => {
  if (word === "certeza") {
    return acc;
  }
  return acc + " " + word;
...
- As primeiras 4 strings são ${filterArray.slice(0, -2)};
...
switch (drinkType) {
  ...
  case "refrigerante":
    console.log(
      "bebida não alcoólica e não fermentada, fabricada industrialmente, á base de água mineral e açúcar."
    );
    break;
  case "suco":
